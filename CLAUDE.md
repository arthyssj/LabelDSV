...
- **`MainWindow.xaml` / `.cs`** — the 3-tab UI (Individual print, Batch print, Config) and all UI event/validation/print-orchestration logic.
- **`ZplBuilder.cs`** (`Construir()`) — the single source of truth for every real printed coordinate. Label is 4x6.5in @ 300dpi (`^PW1200` x `^LL1950`), content rotated 90° (`^A0R` text, `^BCR` barcodes) because the label reads sideways relative to how it feeds into the printer.
